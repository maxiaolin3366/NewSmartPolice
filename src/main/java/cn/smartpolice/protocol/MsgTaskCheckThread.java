...
/**
 * 消息?????检查线?? ???查消息推送任务队??? 若队列不为null则根据任务node信息，在消息?????记录表中添加记录，并在此线程中给在线的节点发???未读消?????知
 *
 * @author maxiaolin
 */
public class MsgTaskCheckThread implements Runnable, ConstParam {
    int sign;
    int messageid;
    ...
    */
			...
			try {
				...
				if (msgTask.getmType() == 1) {
					...
					if (msgTask.getRevUserID() == 1) {
						...
						SysInfo.getInstance().removeMsgTask(msgTask);
						System.out.println("接收者id不正确，已删除节???");
					}
				}
				// type==2为聊天消???
					...
					String type = jsonAnalysis.getValue(msgTask.getContent(), "TYPE");// 字???
					...
					msgRecv.setMsgtype("2");// 2为聊天消???
					msgRecv.setState(0);
					msgRecv.setRecvtime(new Date(msgTask.getmDate()));
					sign = messageDao.insertMessageOfMsgRecv(msgRecv);// 将未读消息插入消息记录????中，返回SIGN
				...
				else if (msgTask.getmType() == 4) {//0 ???有APP用户??2 ???有设备管理员??3 厂商设备用户??4管理员；5全体??有成??
					...
					notice.setSets(set);
					if (msgTask.getRevUserID() == 0) {// 系统通告，发送给???有APP用???
						notice.setRecvtype(1);
						
						messageid = messageDao.insertMessageOfNotice(notice);
						System.out.println(messageid);
						List<UserInfo> user = userDao.findAppuser();//查询??有APP用户id
						MsgRecv msgRecv = new MsgRecv();
						...
						SysInfo.getInstance().removeMsgTask(msgTask);
					}else if(msgTask.getRevUserID() == 2){//设备管理???
						MsgRecv msgRecv = new MsgRecv();
						...
						SysInfo.getInstance().removeMsgTask(msgTask);
					}else if(msgTask.getRevUserID() == 3) {//厂商设备用???
						MsgRecv msgRecv = new MsgRecv();
						...
						SysInfo.getInstance().removeMsgTask(msgTask);
					}else if(msgTask.getRevUserID() == 4) {//管理??
						MsgRecv msgRecv = new MsgRecv();
						...
						SysInfo.getInstance().removeMsgTask(msgTask);
					}else if(msgTask.getRevUserID() == 5) {//全体??有成??
						
					}else{
						System.out.println("不支持的消息类????,删除该消???");
				...
				System.out.println("捕获异常，删除节???"+e.getMessage());
			} finally {
				SysInfo.getMsgTaskQueue().remove(msgTask);
			}
		}
	...
	public byte[] PackPkt(MsgTask msgTask, Object c, UserNode userNode) {
		...
		while (true) {
			...
			seq = rando.nextInt(999999) % 900000 + 100000; // 产生??????6位随机数
			if (seq != userNode.getSntPktId()) {
				userNode.setSntPktId(seq);//产生???个不与上??????动发送报文时相同的发送序???
				break;
			}
		}
		msg.setSeq(seq);
